...
RF24::RF24(uint8_t _cepin, uint8_t _cspin, uint32_t _spi_speed):
  ce_pin(_cepin),csn_pin(_cspin),spi_speed(_spi_speed),p_variant(false), payload_size(8), dynamic_payloads_enabled(false),addr_width(5)//,pipe0_reading_address(0) 
{
  pipe0_reading_address[0]=0;
}
#endif
...
bool RF24::begin(void)
{
  ...
  write_register( CONFIG, 0b00111000 ) ;
...
//KBL
  write_register( EN_RXADDR, 3 );
  write_register( RX_PW_P0, 8);
  write_register( RX_PW_P1, 8);
...
{
#if 0
  printf("%02X", read_register(0));
  printf("%02X", read_register(NRF_STATUS));
  printf("%02X", read_register(FIFO_STATUS));
  printf("%02X", read_register(0x11));
  printf("%02X", read_register(0x12));
  printf("%02X", read_register(0x13));
  printf("%02X", read_register(0x14));
  printf("%02X", read_register(0x15));
  printf("%02X", read_register(0x16));
  fflush(stdout);
#endif

//  if ((read_register(FIFO_STATUS) & (1<<RX_EMPTY)) == 0){
    if (read_register(NRF_STATUS) & (1<<RX_DR)){
